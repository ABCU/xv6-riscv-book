...
and {\tt usertrap()} and {\tt usertrapret()} in {\tt kernel/trap.c}.
...
calls {\tt userret} on the trampoline page.
...
and {\tt copyinstr()} in {\tt kernel/vm.c}.
...
The kernel does not have a way to recover from its own bugs,
so it calls {\tt panic()}, which prints an error message
and then halts. More generally, 
the kernel calls {\tt panic()}
when it encounters a situation disastrous
enough that it cannot proceed.
