...
allocation, which allows allows a program to ask for a huge virtual
...
\lineref{kernel/sysproc.c:/^sys_sbrk/}.  To grow a process's
memory, {\tt sys\_sbrk} just increments the process's size
...
\indextext{copy-on-write (COW) fork}. {\tt fork} promises that the
