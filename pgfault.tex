...
page tables, which opens up a wide range of features without having to
modify applications.  It enables the kernel to implement system calls
more efficiently: for example, copy-on-write fork allows the kernel to
avoid copying the parents memory. It can provide more convenience for
applications programmers: for example, lazy allocation allows an
application to allocate memory lazily without having to predict
upfront exactly how memory it will need.  xv6 doesn't make much use of
this powerful combination; it has only one example: lazy allocation.
This chapter describes this example and provides examples of how
production operating systems use page-fault exceptions for other
features.
...
process, \lstinline{sys_sbrk} just increment the process's size
(\lstinline{myproc()->sz}) by \lstinline{n} and return the old size; it does
  not allocate memory.
...
checks that the faulting address is a valid address, allocates physical memory with {\tt kalloc},
...
allocated page. Then, \lstinline{usertrap} resumes the
