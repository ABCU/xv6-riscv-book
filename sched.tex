...
The two functions have this interface:
...
as a 64-bit number; the only thing they do with it is compare for
...
the condition is shared data, used by both the
...
which in this case is the condition lock. It's a rule that any code
...
\lstinline{wakeup} between the check of the condition (e.g. whether
there's data in the pipe) and the call to \lstinline{sleep}. A call to
...
possibility that another thread might call \lstinline{pipewrite}
...
\section{An illustration of lost wake-ups}
\label{sec:sleep}

To motivate the way \lstinline{sleep} and \lstinline{wakeup} work in xv6,
...
The code above 
would be expensive.  If the producer acts
...
\indexcode{naive_sleep(chan)}
and
\indexcode{naive_wakeup(chan)},
that are similar to the real functions except
they do not demand that a condition lock be held.
But, like \lstinline{sleep},
\lstinline{naive_sleep} yields the CPU so
that it can do other work.
We can change the semaphore implementation to use
\lstinline{naive_sleep}
and
\lstinline{naive_wakeup} (changes highlighted in yellow):
\begin{lstlisting}[numbers=left,firstnumber=200]
  ...
  P(struct semaphore *s)
  {
    while(s->count == 0)    (*@\label{line:test}@*)
      (*@\hl{naive\_sleep(s);}@*)  (*@\label{line:sleep}@*)
    acquire(&s->lock);
    s->count -= 1;
    release(&s->lock);
  }
...
\lstinline{naive_sleep}
and 
\lstinline{naive_wakeup}
interface suffers
from the lost wake-up problem.
% (see 
...
\lstinline{P}
so that its check of the count and its call to \lstinline{naive_sleep}
are both protected by the lock:
\begin{lstlisting}[numbers=left,firstnumber=300]
  ...
  P(struct semaphore *s)
  {
    ...
    while(s->count == 0)    (*@\label{line:test1}@*)
      naive_sleep(s);             (*@\label{line:sleep1}@*)
    s->count -= 1;
    release(&s->lock);
  }
\end{lstlisting}
...
The real \lstinline{sleep} avoids lost wake-ups by requiring the
condition lock as an argument, and releasing the lock after
...
The correct sleep/wakeup scheme is usable as follows (change
...
releases
\lstinline{s->lock}
and puts the consuming process to sleep
