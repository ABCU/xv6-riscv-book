...
to the definition of {\tt struct proc} in {\tt kernel/proc.h},
...
A kernel thread giving up its CPU always calls
\lstinline{sched},
and {\tt sched}'s call to {\tt swtch()} always
switches to the same location in \lstinline{scheduler}, which
(almost) always switches to some kernel thread that previously called
\lstinline{sched}. 
Thus, if one were to print out the line numbers where xv6 
calls {\tt swtch()},
one would observe the following simple pattern:
